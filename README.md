...
Для запуска проекта, откройте Git Bash в папке client(на данный момент доделываю серверную часть) проекта и выполните команду:
...
Изначально проект планировался совместно с моим другом, который занимается бэкэндом и авторизацией через JWT-токены. Однако, на данный момент, в проекте используется "фейк-api" JSON-Server, но сейчас я потихоньку делаю нормальную серверную часть. Тем не менее, у меня есть репозиторий, в котором реализована [JWT-авторизация](https://github.com/KBcHMFollower/AuthWithJBTTokens) с двумя токенами.
